...
class _LiveStreamUserState extends State<LiveStreamUser>
          ...
          (event) {
                ...
                _database
                    ...
                    .then((sosSnapshot) {
                      if (sosSnapshot.exists) {
                        ...
                        final isOnStatusPage = currentRoute.contains(
                          'EmergencyStatusPage',
                        );
                        if (!isOnStatusPage) {
                          _showNotification(
            ...
            try {
              ...
              for (var responderId in data.keys) {
                ...
                final emergencies = Map<String, dynamic>.from(
                  responderValue as Map,
                );

                for (var emergencyId in emergencies.keys) {
                  final emergency = Map<String, dynamic>.from(
                    emergencies[emergencyId] as Map,
                  );
  ...
  void _handleEmergencyCompleted() {
    ...
    if (!isOnStatusPage) {
      ...
      print(
        "📋 User is on EmergencyStatusPage - SOS page will NOT show notification",
      );
    }
  ...
  Future<void> _checkForActiveEmergency() async {
    ...
    try {
      final snapshot = await FirebaseDatabase.instance.ref('assigned').get();
                  ...
                  AnimatedBuilder(
                    ...
                    builder: (context, child) {
                      ...
                      final double wobbleAngle =
                          sin(value * 4 * 3.14159) * 0.08 * (1 - value);
